...
Theorem CONTRACT_CASE1_THM: (* ALSO CASE 2 ?? THINK ABOUT IT *)
...
Proof
  REPEAT GEN_TAC >>
  NTAC 3 STRIP_TAC >>
  UNDISCH_TAC ``REGISTER_CONTRACT sc_tx block_tx reg success`` >>
...
Theorem BLOCKS_CASE1_THM:
  (!block_tx blocks. BLOCKS block_tx blocks ==> 
      (!t. block_tx t = NO_DATA ==> 
        blocks (t+1) = blocks t))
Proof
  REPEAT GEN_TAC >>
  NTAC 3 STRIP_TAC >>
  UNDISCH_TAC ``BLOCKS block_tx blocks`` >>
...
Theorem BLOCKS_CASE2_THM:
  (!block_tx blocks. BLOCKS block_tx blocks ==> 
      (!t x. block_tx t = DATA x ==> 
        blocks (t+1) = x::(blocks t)))
Proof
  REPEAT GEN_TAC >>
  NTAC 4 STRIP_TAC >>
  UNDISCH_TAC ``BLOCKS block_tx blocks`` >>
...
Definition USER_INTERACT_BLOCKCHAIN_def:
  USER_INTERACT_BLOCKCHAIN txs pending_tx success tc1 dtc1
    tc2 dtc2 tc3 dtc3 ch1 ch2 ch3 mine sc_tx reg block_tx
    blocks =
      ((USER_REG_CALL txs pending_tx success) /\
       (BLOCKCHAIN tc1 dtc1 tc2 dtc2 tc3 dtc3 ch1 ch2 ch3 pending_tx mine sc_tx reg success block_tx blocks))
End
...
Definition BLOCKCHAIN_INITAL_VALUES_def:
  BLOCKCHAIN_INITIAL_VALUES tc1 dtc1 tc2 dtc2 tc3 dtc3 ch1
  ch2 ch3 mine sc_tx reg block_tx blocks = (
     (dtc1 0 = tc1) /\ (dtc2 0 = tc2) /\ (dtc3 0 = tc3) /\
     (ch1 0 = NO_DATA) /\ (ch2 0 = NO_DATA) /\ (ch3 0 = NO_DATA) /\
     (mine 0 = NO_DATA) /\ (sc_tx 0 = NO_DATA) /\ 
     (block_tx 0 = NO_DATA) /\ (reg 0 = []) /\ (blocks 0 = [])
  )
End

(* Complete Blockchain system *)

Definition BLOCKCHAIN_SYSTEM_def:
  BLOCKCHAIN_SYSTEM txs pending_tx success tc1 dtc1 tc2 
  dtc2 tc3 dtc3 ch1 ch2 ch3 mine sc_tx reg block_tx blocks
    = ((BLOCKCHAIN_INITIAL_VALUES tc1 dtc1 tc2 dtc2 tc3 
    dtc3 ch1 ch2 ch3 mine sc_tx reg block_tx blocks) /\ 
    (USER_INTERACT_BLOCKCHAIN txs pending_tx success tc1
    dtc1 tc2 dtc2 tc3 dtc3 ch1 ch2 ch3 mine sc_tx reg 
    block_tx blocks))
End
...
(* TRANSACTION --> CHANNEL --> MINING *)
val TRANS_TO_MINE = Define `
    TRANS_TO_MINE pending_tx ch1 tc1 dtc1 mine ch2 =
    ((TRANSACTIONS pending_tx ch1) /\
     (CHANNEL tc1 dtc1 ch1 mine) /\
     (MINING mine ch2))
`
...
(* MINING --> CHANNEL --> REGISTER CONTRACT *)
val MINE_TO_CONTRACT = Define `
    MINE_TO_CONTRACT mine ch2 tc2 dtc2 sc_tx ch3 reg success=
     ((MINING mine ch2) /\ 
     (CHANNEL tc2 dtc2 ch2 sc_tx) /\
     (REGISTER_CONTRACT sc_tx ch3 reg success))
`
...
(* REGISTER CONTRACT --> CHANNEL --> BLOCKS *)
val CONTRACT_TO_BLOCKS = Define `
    CONTRACT_TO_BLOCKS sc_tx ch3 reg success tc3 dtc3 block_tx blocks = 
     ((REGISTER_CONTRACT sc_tx ch3 reg success) /\
     (CHANNEL tc3 dtc3 ch3 block_tx) /\
     (BLOCKS block_tx blocks))
`
...
(* TODO: BLOCKCHAIN_SYSTEM ==> pending_tx t = mine (t+tc1) *)
