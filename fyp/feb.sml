...
val TRANSACTIONS_def = Define `
  TRANSACTIONS tr dtr pending_tx mine =
    !t. (if pending_tx t = [user_data] then
           (if dtr t = 0 then
              (mine t = user_data) /\
              (dtr (t+1) = tr)     /\
              (pending_tx (t+1) = [])
...
(dtr 0 = tr) /\ (pending_tx 0 = [user_data]) /\ (TRANSACTIONS tr dtr pending_tx mine) ==> (!t. t < tr ==> pending_tx t = [user_data])
