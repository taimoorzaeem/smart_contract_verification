...
(* TODO: Prove both the below theorems *)

(* TODO: Add type annotations to some of these variables
         to avoid type mismatchs in proof calculus.
*)
...
Theorem BLOCKCHAIN_CHANNEL_HACKED_1:
  ...
  (USER_INTERACT_BLOCKCHAIN reg_call user_out trans_in trans_out mine_in mine_out contr_in contr_out blocks_in register notify success pending_tx blocks tu dtu tg dtg tmine dtmine tm dtm te dte tb dtb tc dtc) /\
  (CONTRACT_CALL reg_call notify)
    ==> (HACKED_CHANNEL (tu + tc) user_out)
Proof
  RW_TAC std_ss [USER_INTERACT_BLOCKCHAIN_def,BLOCKCHAIN_def,CONTRACT_CALL_def,TRANSACTIONS_def,CHANNEL_def,MINING_def,REGISTER_CONTRACT_def,BLOCKS_def,USER_REG_CALL_def,HACKED_CHANNEL_def,INIT_def]
  METIS_TAC []
QED
...
(* TODO: Add theorems for SCENARIO: 3 *)
